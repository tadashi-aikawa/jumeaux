    ...
    """
             notifies:
               - notifier: jumeaux
                 message: "notify!"
             """,
    [{"name": "req", "method": "GET", "path": "/sample", "headers": {}, "qs": {}}],
    """
             one:
                name: one
                host: http://one   
...
class TestExec:
    @pytest.mark.parametrize(
        "title, config_yml, requests, jumeaux_config_yml, expected_result", [NOT_EMPTY]
    )
    def test_not_use_notify(
        self, send, title, config_yml, requests, jumeaux_config_yml, expected_result
    ):
        ...
        actual: Reqs2ReqsAddOnPayload = Executor(load_yaml(config_yml)).exec(
            payload, JumeauxConfig.from_yaml(jumeaux_config_yml)
        )

        assert expected_result == actual.requests.to_dicts()

    @pytest.mark.parametrize("title, config_yml, requests, jumeaux_config_yml", [EMPTY])
    def test_use_notify(self, send, title, config_yml, requests, jumeaux_config_yml):
        ...
        try:
            Executor(load_yaml(config_yml)).exec(
                payload, JumeauxConfig.from_yaml(jumeaux_config_yml)
            )
        except SystemExit:
